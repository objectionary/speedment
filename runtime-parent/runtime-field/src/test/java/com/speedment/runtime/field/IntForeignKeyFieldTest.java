...
final class IntForeignKeyFieldTest {
    
    private static final Function<BasicEntity, String> FORMATTER = entity -> "" + entity.getVarInt();
    private IntField<BasicEntity, Integer> field;
    private IntField<BasicEntity, Integer> fkField;
    ...
    void setUp() {
        ...
        field = IntForeignKeyField.create(
            BasicEntity.Identifier.VAR_INT,
            BasicEntity::getVarInt,
            BasicEntity::setVarInt,
            fkField,
    ...
    void testInSet() {
        ...
        final Predicate<BasicEntity> t4 = field.in(Stream.of(-1, 1, 2, 3).collect(toSet()));
        final Predicate<BasicEntity> t5 = field.in(Stream.of(Integer.MIN_VALUE, Integer.MAX_VALUE).collect(toSet()));
        final Predicate<BasicEntity> t6 = field.in(Stream.of(1, 2, 3, 4, 5).collect(toSet()));
        final Predicate<BasicEntity> t7 = field.in(Stream.of(100, 101, 102, 103, 104).collect(toSet()));
    ...
    void testNotInSet() {
        ...
        final Predicate<BasicEntity> t4 = field.notIn(Stream.of(-1, 1, 2, 3).collect(toSet()));
        final Predicate<BasicEntity> t5 = field.notIn(Stream.of(Integer.MIN_VALUE, Integer.MAX_VALUE).collect(toSet()));
        final Predicate<BasicEntity> t6 = field.notIn(Stream.of(1, 2, 3, 4, 5).collect(toSet()));
        final Predicate<BasicEntity> t7 = field.notIn(Stream.of(100, 101, 102, 103, 104).collect(toSet()));
    ...
    @com.yegor256.AggregateRepeatedTest(100)
    void getField() {
        final IntField<BasicEntity, Integer> other = field.getField();
        assertNotNull(other);
    }
    ...
    @com.yegor256.AggregateRepeatedTest(100)
    void typemapper() {
        when(column.getDatabaseType()).thenReturn(Integer.class.getName());
        final Type fieldType = field.typeMapper().getJavaType(column);
        assertEquals(int.class.getSimpleName(), fieldType.getTypeName());
    }
