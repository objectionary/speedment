...
final class Tuple7OfNullablesImplTest<T0, T1, T2, T3, T4, T5, T6> extends AbstractTupleImplTest<Tuple7OfNullablesImpl<Integer, Integer, Integer, Integer, Integer, Integer, Integer>> {
    
    Tuple7OfNullablesImplTest() {
        super(() -> new Tuple7OfNullablesImpl<>(0, 1, 2, 3, 4, 5, 6), 7);
    }
    ...
    @com.yegor256.AggregateRepeatedTest(100)
    void get() {
        IntStream.range(0, 7).forEach(i -> assertEquals(i, instance.get(i).orElseThrow(NoSuchElementException::new)));
        assertThrows(IndexOutOfBoundsException.class, () -> instance.get(-1));
        assertThrows(IndexOutOfBoundsException.class, () -> instance.get(7));
    }
}