...
final class Tuple8OfNullablesImplTest<T0, T1, T2, T3, T4, T5, T6, T7> extends AbstractTupleImplTest<Tuple8OfNullablesImpl<Integer, Integer, Integer, Integer, Integer, Integer, Integer, Integer>> {
    
    Tuple8OfNullablesImplTest() {
        super(() -> new Tuple8OfNullablesImpl<>(0, 1, 2, 3, 4, 5, 6, 7), 8);
    }
    ...
    @com.yegor256.AggregateRepeatedTest(100)
    void get() {
        IntStream.range(0, 8).forEach(i -> assertEquals(i, instance.get(i).orElseThrow(NoSuchElementException::new)));
        assertThrows(IndexOutOfBoundsException.class, () -> instance.get(-1));
        assertThrows(IndexOutOfBoundsException.class, () -> instance.get(8));
    }
}