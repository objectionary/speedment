...
final class Tuple5OfNullablesImplTest<T0, T1, T2, T3, T4> extends AbstractTupleImplTest<Tuple5OfNullablesImpl<Integer, Integer, Integer, Integer, Integer>> {
    
    Tuple5OfNullablesImplTest() {
        super(() -> new Tuple5OfNullablesImpl<>(0, 1, 2, 3, 4), 5);
    }
    ...
    @com.yegor256.AggregateRepeatedTest(100)
    void get() {
        IntStream.range(0, 5).forEach(i -> assertEquals(i, instance.get(i).orElseThrow(NoSuchElementException::new)));
